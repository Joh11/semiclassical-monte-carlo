...
* Dimer order SKL
** TODO Integrate the bonds properly in the hamiltonian
use a method =bonds= that takes the hamiltonian and returns a list of
bonds.

- a bond: a struct of two sites
- a site: a struct with the site index, and the (frac) pos
